...
export class RefreshTokenRepository extends BasePostgresRepository<RefreshTokenEntity, RefreshToken> {
  ...
  public async save(entity: RefreshTokenEntity): Promise<RefreshTokenEntity> {
    ...
    const document = await this.clientService.refreshToken.create({ data: pojoEntity });
    entity.id = document.id;
    return entity;
  }

  public async findByTokenId(tokenId: string): Promise<RefreshTokenEntity | null> {
    const document = await this.clientService.refreshToken.findFirst({ where: { tokenId } });
  ...
  public async deleteByTokenId(tokenId: string): Promise<void> {
    ...
    if (document) {
      await this.clientService.refreshToken.delete({ where: { id: document.id } });
    }
  }

  public async deleteExpired() {
    await this.clientService.refreshToken.deleteMany({ where: { expiresIn: { lt: new Date() } } });
  }
}
