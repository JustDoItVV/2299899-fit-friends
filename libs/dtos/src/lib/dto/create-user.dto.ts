...
export class CreateUserDto {
  ...
  @Matches(/^[a-zа-я]+$/giu, { message: UserErrorMessage.NameWrongPattern })
  ...
  @ApiPropertyOptional({
    description: 'Дата рождения, строка в формате ISO',
    example: '1967-07-26T12:00:00.000Z',
  })
  ...
  @ApiPropertyOptional({
    description: 'Описание, текст с общей информацией о пользователе',
    example: 'Длинное описание',
  })
  @MaxLength(UserDescriptionLength.Max, {
    message: UserErrorMessage.DescriptionMaxLength,
  })
  @MinLength(UserDescriptionLength.Min, {
    message: UserErrorMessage.DescriptionMinLength,
  })
  @IsString({ message: UserErrorMessage.NotString })
  @IsOptional()
  ...
  @ApiProperty({
    ...
    example: [TrainingType.Running],
    enum: TrainingType,
    isArray: true,
  })
  @Validate(ArrayMinLengthByUserRole)
  @IsEnum(TrainingType, { each: true })
  @ArrayMaxSize(TRAINING_TYPE_LIMIT, {
    message: UserErrorMessage.TrainingTypeMaxSize,
  })
  ...
  @ApiPropertyOptional({
    description: `Только для роли "${UserRole.User}": Время на тренировку"`,
    example: TrainingDuration.Eighty,
  ...
  @ApiPropertyOptional({
    description: `Только для роли "${UserRole.User}": Количество калорий для сброса"`,
    example: CaloriesTargetLimit.Max,
  })
  ...
  @ApiPropertyOptional({
    description: `Только для роли "${UserRole.User}": Количество калорий для траты в день"`,
    example: CaloriesPerDayLimit.Max,
  })
  ...
  @ApiPropertyOptional({
    description: `Только для роли "${UserRole.User}": Флаг готовности пользователя к приглашениям на тренировку"`,
  })
  @IsBoolean()
  ...
  @ApiPropertyOptional({
    description: `Только для роли "${UserRole.Trainer}": Текст с описание заслуг тренера`,
    example: 'Большие заслуги',
  })
  ...
  @ApiPropertyOptional({
    description: `Только для роли "${UserRole.Trainer}": Флаг готовности проводить индивидуальные тренировки"`,
  })
  @IsBoolean()
